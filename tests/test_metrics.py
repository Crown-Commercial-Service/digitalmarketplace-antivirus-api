...
class TestMetrics(BaseApplicationTest):
    ...
    def test_metrics_page_accessible(self):
        metrics_response = self.get_authorized_client().get('/_metrics')
    ...
    def test_metrics_page_contents(self):
        ...
        assert (
            b'http_server_requests_total{code="200",host="127.0.0.1:5008",method="GET",path="/_metrics"}'
        ) in results
...
class TestMetricsPageRegistersPageViews(BaseApplicationTest):
    ...
    def test_metrics_page_registers_page_views(self):
        expected_metric_name = (
            b'http_server_requests_total{code="200",host="127.0.0.1:5008",method="GET",path="/"}'
        )

        res = self.get_authorized_client().get('/')
    ...
    def test_metrics_page_registers_multiple_page_views(self):
        ...
        initial_metrics_response = self.get_authorized_client().get('/_metrics')
